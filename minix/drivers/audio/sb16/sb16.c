...
static void dsp_dma_setup(phys_bytes address, int count, int DmaMode) {
	...
	if(DspBits == 8) {   /* 8 bit sound */
		...
		pv_set(pvb[3], DMA8_ADDR, (uint8_t)(address >>  0)); /* Low_byte of address */
		pv_set(pvb[4], DMA8_ADDR, (uint8_t)(address >>  8)); /* High byte of address */
		pv_set(pvb[5], DMA8_PAGE, (uint8_t)(address >> 16)); /* 64K page number */
		pv_set(pvb[6], DMA8_COUNT, (uint8_t)(count >> 0));   /* Low byte of count */
		pv_set(pvb[7], DMA8_COUNT, (uint8_t)(count >> 8));   /* High byte of count */
		...
		pv_set(pvb[6], DMA16_COUNT, (uint8_t)(count >> 1));    /* Low byte of count */
		pv_set(pvb[7], DMA16_COUNT, (uint8_t)(count >> 9));    /* High byte of count */
